...
export interface ContributeRefs {
...
const TRAIT_LIST_FOR: Partial<Record<ProfileFieldKey, keyof ContributeRefs>> = {
  clans: "clans",
...
/** Sections de la palette : le champ se choisit par rubrique, pas dans une liste de 25. */
const PALETTE: { title: string; keys: ProfileFieldKey[] }[] = [
...
export function ContributeIntel({
  ...
  const valueEditor = useMemo(() => {
    if (!fieldKey || noneConfirmed) return null;
    ...
    switch (fieldKey) {
      case "lastName":
        return <input aria-label="Nom" className={input} maxLength={80} value={(value as string) ?? ""} onChange={(e) => setValue(e.target.value)} />;
      case "details":
      case "strengths":
      case "weaknesses":
        return <textarea aria-label={PROFILE_FIELD_LABELS[fieldKey]} className={`${input} min-h-[6rem]`} maxLength={10_000} value={(value as string) ?? ""} onChange={(e) => setValue(e.target.value)} />;
      case "sex":
        return (
          <select aria-label="Sexe" className={input} value={(value as string) ?? ""} onChange={(e) => setValue(e.target.value || undefined)}>
      ...
      case "lifeStatus":
        return (
          <select aria-label="État vital" className={input} value={(value as string) ?? ""} onChange={(e) => setValue(e.target.value || undefined)}>
            <option value="">— choisir —</option>
      ...
      case "faction":
        return (
          <select aria-label="Faction" className={input} value={(value as string) ?? ""} onChange={(e) => setValue(e.target.value || undefined)}>
            <option value="">— choisir —</option>
      ...
      case "rank":
        return (
          <select aria-label="Grade" className={input} value={(value as string) ?? ""} onChange={(e) => setValue(e.target.value || undefined)}>
            <option value="">— choisir —</option>
      ...
      case "ninjaClass":
        return (
          <select aria-label="Classe" className={input} value={(value as string) ?? ""} onChange={(e) => setValue(e.target.value || undefined)}>
            <option value="">— choisir —</option>
      ...
      case "skinTone": {
        ...
        return (
          <ReferencePicker legend={PROFILE_FIELD_LABELS[fieldKey]} hideLegend options={list}
            selected={value ? [value as string] : []} onChange={(ids) => setValue(ids[ids.length - 1])} />
        );
      }
      case "eyeColor": {
        ...
        return (
          <div className="space-y-2">
            <ReferencePicker legend="Œil 1" options={refs.eyeColors}
              ...
              onChange={(ids) => setValue({ ...v, primaryId: ids[ids.length - 1] })} />
            <ReferencePicker legend="Œil 2 (hétérochromie, facultatif)"
              ...
              onChange={(ids) => setValue({ ...v, secondaryId: ids[ids.length - 1] ?? null })} />
          </div>
        );
      }
      case "height": {
        ...
        return (
          <div className="flex items-center gap-2">
            <input type="number" min={30} max={400} placeholder="min (cm)" aria-label="Taille minimum" className={input}
              value={v.minCm ?? ""} onChange={(e) => setValue({ minCm: e.target.value ? Number(e.target.value) : null, maxCm: v.maxCm ?? null })} />
            <span aria-hidden className="text-ink-faint">–</span>
            <input type="number" min={30} max={400} placeholder="max (cm)" aria-label="Taille maximum" className={input}
              value={v.maxCm ?? ""} onChange={(e) => setValue({ minCm: v.minCm ?? null, maxCm: e.target.value ? Number(e.target.value) : null })} />
          </div>
        );
      }
      case "age": {
        ...
        return (
          <div className="space-y-2">
            <select aria-label="Mode" className={input} value={v.mode ?? "AGE_AT_REFERENCE"} onChange={(e) => setValue({ mode: e.target.value })}>
            ...
            {v.mode === "AGE_RANGE_AT_REFERENCE" ? (
              <div className="flex items-center gap-2">
                <input type="number" min={0} placeholder="min" aria-label="Âge minimum" className={input} value={v.min ?? ""} onChange={(e) => setValue({ ...v, min: e.target.value ? Number(e.target.value) : null })} />
                <span aria-hidden className="text-ink-faint">–</span>
                <input type="number" min={0} placeholder="max" aria-label="Âge maximum" className={input} value={v.max ?? ""} onChange={(e) => setValue({ ...v, max: e.target.value ? Number(e.target.value) : null })} />
              </div>
            ) : (
              <input type="number" min={0} placeholder="Âge en années" aria-label="Âge" className={input} value={v.years ?? ""} onChange={(e) => setValue({ mode: "AGE_AT_REFERENCE", years: e.target.value ? Number(e.target.value) : null })} />
            )}
          </div>
        );
      }
        ...
        return (
          <div className="space-y-2">
            <input aria-label="Nom de la technique" className={input} maxLength={120} placeholder="Nom de la technique" value={t.name}
              onChange={(e) => setValue([{ ...t, name: e.target.value }])} />
            <div className="grid gap-2 sm:grid-cols-2">
              <select aria-label="Type de jutsu" className={input} value={t.jutsuTypeId ?? ""} onChange={(e) => setValue([{ ...t, jutsuTypeId: e.target.value || null }])}>
                <option value="">Type — inconnu</option>
                {refs.jutsuTypes.map((j) => <option key={j.id} value={j.id}>{j.label}</option>)}
              </select>
              <select aria-label="Rang" className={input} value={t.rank ?? ""} onChange={(e) => setValue([{ ...t, rank: e.target.value || null }])}>
            ...
            <input aria-label="Description courte" className={input} maxLength={1000} placeholder="Description courte (facultative)" value={t.shortDescription ?? ""}
              onChange={(e) => setValue([{ ...t, shortDescription: e.target.value }])} />
          </div>
        );
      }
      default:
  ...
  }, [fieldKey, noneConfirmed, value, refs]);

  const canNone = fieldKey ? fieldKey !== "age" && fieldKey !== "techniques" : false;
